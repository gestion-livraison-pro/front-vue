              ...
              <div class="card">
                <div class="card-body">
                  ...
                  <div class="row">
                    ...
                    <SelectDropdown v-model="VilleId" :value="formData.VilleId"
                              ...
                              <li class="list-group-item">
                                <div class="d-flex row justify-content-between align-items-center">
                                  <p class="d-flex justify-content-between align-items-center col-md-5 mb-3">
                                    <span class="col-6">Quantité Stock</span> <span class="badge bg-success mx-3">{{
                                  ...
                                  <p class="d-flex jsutify-content-between align-items-center mb-3 col-md-5">
                                    <span class="col-6">Quantité</span>
                                    <input_text v-model="selected_product.qte" type="number" Class_Content="col-6 "
...
import { addData, fetchData, getRole, updateData } from '../../composition/crud.js';

import { useRoute } from 'vue-router'; // Import useRoute

const route = useRoute(); // Create a reference to the route object
...
onMounted(async () => {
  ...
  // Get order
  const order = await fetchData(`https://api.gestion-livraison.pro/api/order/${route.params.id}/get`);
  console.log(order);

  if (order) {
    // Destructure the response and assign it to formData
    formData.value = { ...formData.value, ...order };


    // Iterate through products_getted and add matching products to cartItems
    for (const product_getted of order.products) {
      // Check if the product_getted is already in cartItems
      const isInCart = cartItems.value.some((item) => item.id === product_getted.id);

      console.log("isInCart" + isInCart)

      // If it's not in cartItems, add it
      if (!isInCart) {
        const matchingProduct = data.value.find((product) => product.id === product_getted.id);

        console.log("matchingProduct" + matchingProduct)

        // Ensure that a matching product is found in the data array
        if (matchingProduct) {
          // Add the matching product to cartItems
          cartItems.value.push(matchingProduct);
        }
      }
    }
...
const submitForm = async () => {
  ...
  await updateData(`https://api.gestion-livraison.pro/api/order/${route.params.id}/update`, formData.value);
};
