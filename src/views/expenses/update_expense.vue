                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <form @submit.prevent="submitForm">
                            ...
                            <button_simple type="submit" Class_Content="col-md-12 mt-4 d-flex" btn_text="Modifier Expense" />
...
export default {
  ...
  async mounted() {
    const expenseID = this.$route.params.id; // Retrieve the customer ID from the URL
      const expenseData = await getRow(`https://api.gestion-livraison.pro/api/expense/${expenseID}/get`);
      this.formData = { ...expenseData }; // Populate the form with the retrieved customer data
  },
  methods: {
         async submitForm() {
      const expenseID = this.$route.params.id; // Retrieve the customer ID from the URL
      await updateData(`https://api.gestion-livraison.pro/api/expense/${expenseID}/update`, this.formData);
    },
  },
