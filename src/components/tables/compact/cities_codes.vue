...
<div v-if="data.length > 0" class="table-responsive">
  ...
  <div id="modalAdd" class="modal fade" tabindex="-1" aria-labelledby="modalAdd" aria-hidden="true">
    <form @submit.prevent="submitFormAdd">
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title mt-0" id="modalAdd">Nouvelle Code</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body">
  ...
  <Supprimer :apiUrl="`https://api.gestion-livraison.pro/api/city/code/${delete_row_id}/delete`"
...
export default {
 ...
 methods: {
    ...
    async submitFormUpdate() {
      ...
      const return_data = await updateData(`https://api.gestion-livraison.pro/api/city/code/${CityCodeID}/update`, this.formData);
      this.formData = {
        id: '',
        Nom: '',
      };
    ...
    async submitFormAdd() {
      const res = await addData("https://api.gestion-livraison.pro/api/city/code/new", this.formData);
      this.formData = {
        Nom: '',
      };
      $('#modalAdd').modal('hide');
    ...
    async getFetchData(filters = null) {
      ...
      if(this.data.length>0) this.tableFields = Object.keys(response.data[0]);
      this.loading = false
    },
  },
  mounted() {
