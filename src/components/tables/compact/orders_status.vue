<template>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <SelectDropdown v-model="PhaseId" :value="formData.PhaseId"
              apiUrl="https://api.gestion-livraison.pro/api/order/phases" fieldName="Nom"
              first_option="Selectionner la Phase" label="Phase" Class_Content="col-md-12 mt-4"
              @update:value="formData.PhaseId = $event" />
            <input_text v-model="Nom" label="Statut" type="text" Class_Content="col-md-12 mt-4" :value="formData.Nom"
              @update:value="formData.Nom = $event" />

            <input_text v-model="Coleur" label="Coleur" type="color" Class_Content="col-md-12 mt-4"
              :value="formData.Coleur" @update:value="formData.Coleur = $event" />
  ...
  <Supprimer :apiUrl="`https://api.gestion-livraison.pro/api/order/status/${delete_row_id}/delete`"
...
export default {
  ...
  methods: {
    ...
    async submitFormUpdate() {
      const StatusID = this.formData.id; // Retrieve the customer ID from the URL
      const return_data = await updateData(`https://api.gestion-livraison.pro/api/order/status/${StatusID}/update`, this.formData);
      
      this.formData.Phase = ""
      this.formData.PhaseId = ""
      this.formData.Coleur = ""
      this.formData.id = ""
    ...
    openUpdateModal(StatusData) {
      console.log(StatusData)
      this.formData = { ...StatusData }; // Make a copy of cityData to avoid reactivity issues
      $('#modalUpdate').modal('show');
      // Show the modal using JavaScript if you are using Bootstrap
      // Example: $('#modal').modal('show');
    },
    async submitFormAdd() {
      const res = await addData("https://api.gestion-livraison.pro/api/order/status/new", this.formData);
      this.formData.Phase = ""
      this.formData.PhaseId = ""
      this.formData.Coleur = ""
      this.formData.id = ""
