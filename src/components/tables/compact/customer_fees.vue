...
<template>
  ...
  <div v-if="data.length > 0" class="table-responsive">
      <table class="table table-bordered">
        ...
        <tbody>
          <tr v-for="item in data" :key="item.id">
            <td v-for="field in tableFields" :key="field"><span class="header_side">{{field}} : </span> {{ item[field] }} <span v-if="field ==='Livre' || field ==='Retourne' || field ==='Refuse'"> DH</span></td>
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title mt-0" id="myModalLabel">Modifier Tarif Client</h5>
              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
  
              <SelectDropdown :value="formData.VilleId" apiUrl="https://api.gestion-livraison.pro/api/cities"
                fieldName="Nom" first_option="Selectionner une Ville" label="Ville" Class_Content="col-md-12 mt-4"
                @update:value="formData.VilleId = $event" />
  
              <SelectDropdown :value="formData.ClientId" apiUrl="https://api.gestion-livraison.pro/api/customers"
                fieldName="Nom" first_option="Selectionner un Client" label="Clients" Class_Content="col-md-12 mt-4"
                @update:value="formData.ClientId = $event" />
  
  
              <div class="row">
                <input_text v-model="Livre" label="Frais livré" type="text" Class_Content="col-md-6 mt-4"
                  :value="formData.Livre" @update:value="formData.Livre = $event" />
  
                <input_text v-model="Refuse" label="Frais refusé" type="text" Class_Content="col-md-6 mt-4"
                  :value="formData.Refuse" @update:value="formData.Refuse = $event" />
  
                <input_text v-model="Retourne" label="Frais Retourne" type="text" Class_Content="col-md-12 mt-4"
                  :value="formData.Retourne" @update:value="formData.Retourne = $event" />
              </div>
            ...
            <div class="modal-footer">
              ...
              <button type="submit" class="btn btn-primary waves-effect waves-light" data-bs-dismiss="modal">Modifier</button>
            </div>
          </div><!-- /.modal-content -->
        </div><!-- /.modal-dialog -->
      </form>
    ...
    <div id="modalAdd" class="modal fade" tabindex="-1" aria-labelledby="modalAdd" aria-hidden="true">
      <form @submit.prevent="submitFormAdd">
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title mt-0" id="modalAdd">Nouvelle Tarif Client</h5>
              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
              <SelectDropdown :value="formData.VilleId" apiUrl="https://api.gestion-livraison.pro/api/cities"
            ...
            <div class="modal-footer">
              <button type="button" class="btn btn-secondary waves-effect waves-light"
                data-bs-dismiss="modal">Fermer</button>
              <button type="submit" class="btn btn-primary waves-effect waves-light" data-bs-dismiss="modal">Ajouter</button>
            </div>
          </div><!-- /.modal-content -->
        </div><!-- /.modal-dialog -->
      </form>
    ...
    <Supprimer :apiUrl="`https://api.gestion-livraison.pro/api/customer/fees/${delete_row_id}/delete`"
  ...
  export default {
    ...
    methods: {
      ...
      async submitFormUpdate() {
        const FeesId = this.formData.id; // Retrieve the customer ID from the URL
        console.log(FeesId)
        console.log(this.formData)
  
        const return_data = await updateData(`https://api.gestion-livraison.pro/api/customer/fees/${FeesId}/update`, this.formData);
        if (return_data.data) {
          this.formData = {
            id: '',
            ClientId: '',
      ...
      async submitFormAdd() {
        const res = await addData("https://api.gestion-livraison.pro/api/customer/fees/new", this.formData);
        console.log(res)
        if (res.data) {
          this.formData = {
            id: '',
      ...
      openUpdateModal(FeesData) {
        this.formData = { ...FeesData }; // Make a copy of cityData to avoid reactivity issues
