<template>
    ...
    <table class="table table-bordered">
      <thead class="bg-light">
        <tr>
          <th>Ref</th>
          <th>Montant</th>
          <th v-if="Role != 'customer'">Client</th>
          <th>Colis</th>
          <th>Cloturé</th>
          <th>Versé</th>
          <th>Action</th>
      ...
      <tbody>
        <tr v-for="item in data" :key="item.id">
          <td><span class="header_side">Ref : </span> <a @click="GenerateInvoice(item.id)" href="" data-bs-toggle="modal">{{ item.Ref }}</a></td>
          <td><span class="header_side">Montant : </span> {{ item.Montant }}</td>
          <td v-if="Role != 'customer'"><span class="header_side">Client : </span> {{ item.Client }}</td>
          <td><span class="header_side">Colis : </span> {{ item.Colis }}</td>
          <td><span class="header_side">Cloturé : </span>
            <checkd v-if="checkModule(requiredModule_Cloturé) && Role != 'customer'" v-model="close.Cloturé"
              :value="close.Cloturé" Class_Content="col-md-1 mb-1 mb-sm-0" :id_check="`cl-${item.Ref}`"
              :is_checked="item.Cloturé" :checked="item.Cloturé === 1" @update:value="formData.Cloturé = $event"
              @change="toggleCloture(item)" />
            ...
            <div v-else :class="`badge ${item.Cloturé === 1 ? 'bg-success ' : 'bg-danger '}`">{{ item.Cloturé === 1 ? "Oui" :
              "Non" }} </div>
          ...
          <td><span class="header_side">Versé : </span>
            <checkd v-if="checkModule(requiredModule_Versé)" v-model="paid.Versé" :value="paid.Versé"
              ...
              :checked="item.Versé === 1" @update:value="paid.Versé = $event" @change="togglepaid(item)" />

            <div v-else :class="`badge badge-success ${item.Versé === 1 ? 'bg-success ' : 'bg-danger '}`">{{ item.Versé === 1 ?
              "Oui" : "Non" }}</div>
          </td>
          ...
          <td class="table_actions">
            <div class="d-flex align-items-center order-actions">
              <button @click="GenerateInvoice(item.id)" type="button" class="btn bg-light btn-sm waves-effect mx-1"><i
                  class="fas fa-file-invoice"></i></button>
              <button v-if="Role!='customer'" type="button" class="btn bg-light btn-sm waves-effect mx-1"><i
                  class="fas fa-pencil-alt"></i></button>
              <button v-if="Role!='customer'" type="button" class="btn bg-light btn-sm waves-effect mx-1" @click="delete_row_id = item.id;"
  ...
  <Supprimer :apiUrl="`https://api.gestion-livraison.pro/api/customer/invoice/${delete_row_id}/delete`"
...
import { fetchDataPagination, updateData, checkPermission, getRole, GenerateInvoicePdf } from '../../composition/crud.js';
...
export default {
  ...
  /* methods: {
   async change_close() {
     await updateData(`https://api.gestion-livraison.pro/api/customer/invoice/${deliveryman_id}/close/update`, this.close);
   },
   async change_paid() {
     await updateData(`https://api.gestion-livraison.pro/api/customer/invoice/${deliveryman_id}/paid/update`, this.paid);
   },
 },*/
  methods: {
    loadPage(nbr) {
    ...
    async toggleCloture(item) {
      ...
      await updateData(`https://api.gestion-livraison.pro/api/customer/invoice/${item.id}/close/update`, { Cloturé: item.Cloturé });
    },
    async togglepaid(item) {
      item.Versé = item.Versé === 1 ? 0 : 1;
      await updateData(`https://api.gestion-livraison.pro/api/customer/invoice/${item.id}/paid/update`, { Versé: item.Versé });
    },
    checkModule(module) {
